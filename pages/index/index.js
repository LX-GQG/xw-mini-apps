...
Page({
  ...
  onBtnClick(e) {
    ...
    db.collection('product').where({
      ...
      const figure = res.data[0].productLines
      db.collection('product_line').where({
        _id: db.command.in(figure)
      }).get().then(res => {
        const figureItem = res.data
        this.setData({
          figure: figureItem
        })
      })
    })
    // if (this.data.swiperItems[0].product == e.currentTarget.dataset.product) {
    //     this.setData({
    //       figure: this.data.swiperItems[0].figure,
    //       username: this.data.swiperItems[0].figure.username
    //     })
    //   }
    //   if (this.data.swiperItems[1].product == e.currentTarget.dataset.product) {
    //     this.setData({
    //       figure: this.data.swiperItems[1].figure,
    //       username: this.data.swiperItems[1].figure.username
    //     })
    //   }
    //   if (this.data.swiperItems[2].product == e.currentTarget.dataset.product) {
    //     this.setData({
  ...
  clickProduct(e) {
    ...
    if(e.currentTarget.dataset.id == this.data.figure[0].id) {
      let Id = this.data.figure[0].name
      wx.navigateTo({
        url: `/pages/selectPeople/index?id=${Id}`,
      })
    } else {
      if(e.currentTarget.dataset.id == this.data.figure[1].id) {
        let Id = this.data.figure[1].name
        wx.navigateTo({
          url: `/pages/selectPeople/index?id=${Id}`,
        })
      } else {
        let Id = this.data.figure[2].name
        wx.navigateTo({
          url: `/pages/selectPeople/index?id=${Id}`,
        })
      }
